...
app.get('/searchtwitter', function(req, res){
	twitter.get('search/tweets', { q: req.query.search, count : 5 }, function(response){
		res.send(response)
	});
});
...
app.get('/search', function(req, res){
	// get my search term from query string or message body
	// search through dummy data (the "data" variable) to find my search term
	// send result back to the client

	// OR
	fs.readFile(__dirname + '/myDummyData.json', function(err, data){
		// get my search term from query string or message body
		// search through dummy data to find my search term
		// send result back to the client
	});
